...
function WriteNewFix() {
  ...
  useEffect(() => {
    window.kakao.maps.load(() => {
      ...
      /*Cannot read properties of null (reading 'panTo')오류 방지*/
  ...
  const searchAddress = () => {
    ...
    let callback = function (result, status) {
      if (status === window.kakao.maps.services.Status.OK) {
        ...
        let detailAddr = result[0].address.address_name; //주소
        const latitude = result[0].y; //위도
        const longitude = result[0].x; //경도

        setLatitude(latitude);
        setLongitude(longitude);
        setAddrInput(detailAddr);
    ...
    geocoder.addressSearch(addrInput, callback);
    /*
    if (window.daum && window.daum.Postcode) {
      new window.daum.Postcode({
        //addrData는 주소 검색 결과를 콜백 함수에서 전달받는 매개변수
    ...
    <>
      <form onSubmit={formOnSubmit}>
        <ScDiv>
          ...
          <ScDivFileUpload>
            <input name="selectedFile" id="searchAddrPop" onClick={searchAddressPopup}></input>
            <label htmlFor="searchAddrPop">주소 검색</label>
          </ScDivFileUpload>
